...
def main(args: argparse.Namespace):
    ...
    train_source_dataset = source_dataset(root=args.source_root, split = 'train', transforms=src_train_transform,image_size=image_size, heatmap_size=heatmap_size)
    print("loading indexes from disk array")
    hard_synth = list(np.load("/pfs/rdi/cei/algo_train/shaarju/KePSI/evalsket/experiments_arjun/hard_VAE_SURREAL_8.npy"))
#     erd_synth = list(np.load("/pfs/rdi/cei/algo_train/shaarju/KePSI/evalsket/experiments_arjun/easy_VAE_SURREAL_8.npy"))
#     mrd_synth = list(np.load("/pfs/rdi/cei/algo_train/shaarju/KePSI/evalsket/experiments_arjun/med_VAE_SURREAL_9.npy"))
#     samples_all = list(np.load("/pfs/rdi/cei/algo_train/shaarju/KePSI/evalsket/experiments_arjun/easy_synth_11p_12j_0.97_train_2p.npy"))
#     print(len(samples_all))
    print("INDEXES LOADED")
    train_source_dataset.samples_index = list(hard_synth)
    print("INDEXES CHANGED")
    print(len(train_source_dataset.samples_index))
...
def pretrain(train_source_iter, train_target_iter, student, style_net, criterion, stu_optimizer, epoch: int, visualize, args: argparse.Namespace):
    ...
    for i in range(args.iters_per_epoch):
        ...
        _,x_s, label_s, weight_s, meta_s = next(train_source_iter)
        x_s = x_s.to(device)
...
def validate(val_loader, model, criterion, visualize, args: argparse.Namespace):
    ...
    with torch.no_grad():
        ...
        for i, (_, x, label, weight, meta) in enumerate(val_loader):
            x = x.to(device)
